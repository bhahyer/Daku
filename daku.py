...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.id == ADMIN_ID:
        help_message = (
            ...
            "/offusers - Users Ka Attack Rokne Ke Liye\n"
            "/onusers - Users Ko Attack Ke Liye Allow Karo\n"
        )
...
async def bgmi(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id != ADMIN_ID and user_id in attack_cooldown:
...
async def offusers(update: Update, context: ContextTypes.DEFAULT_TYPE):
    global MAINTENANCE_MODE

    if update.effective_user.id != ADMIN_ID:
        await update.message.reply_text("🚫 *Only admin can use this command.*", parse_mode="Markdown")
        return

    MAINTENANCE_MODE = True
    await update.message.reply_text("⚠️ *Bot is now in Maintenance Mode. Only admin can attack.*", parse_mode="Markdown")

async def onusers(update: Update, context: ContextTypes.DEFAULT_TYPE):
    global MAINTENANCE_MODE
    ...
    MAINTENANCE_MODE = False
    await update.message.reply_text("✅ *Bot is now live for all users.*", parse_mode="Markdown")
...
def main():
    ...
    application.add_handler(CommandHandler("offusers", offusers))
    application.add_handler(CommandHandler("onusers", onusers))
